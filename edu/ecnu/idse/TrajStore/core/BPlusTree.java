...
public class BPlusTree<T extends Comparable<T>, V> implements Serializable{
    ...
    /*
        given two existed keys
     */
    public List<V> RangeSearch(T minValue, T maxValue){
        List<V> results = new ArrayList<V>();
        DFSRangeSearch(root,results,minValue,maxValue);
        return results;
    }

    // Tested!
    public void DFSRangeSearch(Node<T,V> node, List<V> results, T minValue, T maxValue){
        T curKey = null;
        if(node instanceof LeafNode){
            ...
            if(maxValue.compareTo((T)internalNode.keys[0])<0){
                DFSRangeSearch(internalNode.pointers[0],results,minValue,maxValue);
            }else if(minValue.compareTo((T)internalNode.keys[curSize -1]) >=0){
                DFSRangeSearch(internalNode.pointers[curSize],results,minValue,maxValue);
            }else{
                //在其中某些节点上
                ...
                for(int i=upperBoundKeyIndex;i>=0;i--){
                    curKey = (T) node.keys[i];
                ...
                for(int j=lowerBoundKeyIndex;j <= upperBoundKeyIndex ;j++){
                    DFSRangeSearch(internalNode.pointers[j+1],results,minValue,maxValue);
                }
            }
        }
    }
      /*
      @minValue the search lower bound
      @maxValue the search upper bound
       to search the values whose keys with lower bounds of minValue than the given key first;
       and choose the smallest one
       */

    public List<V> RangeSearchOnLowerBoundKey(T minValue, T maxValue){
        List<V> results = new ArrayList<V>();
        RangeSearchOnLowerBoundKey(this.root,results,minValue,maxValue);
        return results;
    }

    public void RangeSearchOnLowerBoundKey(Node<T,V> node, List<V> results, T minValue, T maxValue){
        T curKey = null;
        if(node instanceof LeafNode){
            LeafNode<T,V> leafNode = (LeafNode<T,V>)node;
            // find lower bound for the maxValue
            int maxValueLowerBound = leafNode.size -1;
            for(int i = leafNode.size-1 ;i >= 0;i--){
                curKey = (T)node.keys[i];
                if(curKey.compareTo(maxValue)<=0 ){
                    maxValueLowerBound = i;
                    break;
                }
            }
            // find lower bound for the minValue
            int minValueLowerBound = 0;
            for(int i = maxValueLowerBound ; i >= 0; i--){
                curKey = (T)node.keys[i];
                if(curKey.compareTo(minValue)<=0 ){
                    minValueLowerBound = i;
                    break;
                }
            }
            // get all values between the two bounds
            for(int i= minValueLowerBound;i<=maxValueLowerBound;i++){
                results.add((V)leafNode.values[i]);
            }
        }else{
            InternalNode<T,V> internalNode = (InternalNode<T,V>) node;
            int curSize = internalNode.size;
            if(maxValue.compareTo((T)internalNode.keys[0])<0){
                RangeSearchOnLowerBoundKey(internalNode.pointers[0],results,minValue,maxValue);
            }else if(minValue.compareTo((T)internalNode.keys[curSize -1]) >=0){
                RangeSearchOnLowerBoundKey(internalNode.pointers[curSize],results,minValue,maxValue);
            }else{
                //在其中某些节点上
                //找出大于等于 minValue的key 和 小于等于 maxValue的key

                int upperBoundKeyIndex = node.size-1;
                int lowerBoundKeyIndex = node.size-1;
                ...
                for(int j=lowerBoundKeyIndex;j <= upperBoundKeyIndex;j++){
                    RangeSearchOnLowerBoundKey(internalNode.pointers[j+1],results,minValue,maxValue);
                }
            }
        }
    }
    ...
    /*
       to search the values whose keys is less than the given key first;
       and choose the smallest one.
       Test!
     */
    public V lowerBoundQuery(T qKey){
        return DFSLowerBoundQuery(this.root,qKey);
    }

    public V DFSLowerBoundQuery(Node<T,V> node,T qkey){
        T curKey = null;
        V result = null;
        if(node instanceof LeafNode){
            LeafNode<T,V> leafNode = (LeafNode<T,V>)node;
            for(int i = leafNode.size-1 ;i >= 0;i--){
                curKey = (T)node.keys[i];
                if(curKey.compareTo(qkey)<=0 ){ ///
                    result = (V)leafNode.values[i];
                    break;
                }
            }
            ...
            InternalNode<T,V> internalNode = (InternalNode<T,V>) node;
            int curSize = internalNode.size;
            if(qkey.compareTo((T)internalNode.keys[0])<0){
                return   DFSLowerBoundQuery(internalNode.pointers[0],qkey);
            }else if(qkey.compareTo((T)internalNode.keys[curSize -1]) >=0){
                return   DFSLowerBoundQuery(internalNode.pointers[curSize],qkey);
            }else{
                //在其中某个节点上
                for(int j=0;j < internalNode.size -1;j++){
                    curKey = (T)internalNode.keys[j];
                    T nextKey = (T)internalNode.keys[j+1];
                    if(curKey.compareTo(qkey)<=0 && nextKey.compareTo(qkey)>0)
                        return   DFSLowerBoundQuery(internalNode.pointers[j+1],qkey);
                }
            }
        }
        return result;
    ...
    public void BFSTraverse(){
        ...
        if(root instanceof LeafNode){
            for(int i=0;i<root.size;i++){
                System.out.println(root.keys[i] + "\t" +((LeafNode) root).values[i]);
            }
            return;
        }
    ...
    public static void testBPListSearialize(){
    ...
    /*
     test construction and RangeSearch and BFS traverse
     */
    public static void TreeConstructionTest(){
        BPlusTree<Integer,String> myTree = new BPlusTree<>(4);
        int max =100;
        long start = System.currentTimeMillis();
        for (int i = 0; i<max; i++){
            //     System.out.println("add "+i);
        ...
        BPlusTree<Integer,String> myTree1 = new BPlusTree<>(4);
        max =1000;
        start = System.currentTimeMillis();
        for (int i = 250; i<max; i++){
            //     System.out.println("add "+i);
            myTree1.insert(i,String.valueOf(i));
            //    myTree.BFSTraverse();
        }
        System.out.println("B plus tree construction cost: "+(System.currentTimeMillis() - start)/1000 +" seconds.");
        System.out.println("the height of the tree "+myTree.height());
        List <String> res1 = myTree.RangeSearch(2001,2008);
        for(String s: res1){
            System.out.println(s);
        }

        BPlusTree<Integer,String> [] forest = new BPlusTree[2];
        forest[0] = myTree;
        forest[1] = myTree1;
        for(int i=0;i<forest.length;i++){
            forest[i].BFSTraverse();
        }
    }
    ...
    public static void main(String[] args){
       ...
       System.out.println(myTree.lowerBoundQuery(101));
       List<String> r1 = myTree.RangeSearch(79,99);
        System.out.println("range query");
        for (String s:r1){
            System.out.print(s+" ");
        }
        System.out.println();
        System.out.println("range query with lower bound");
        List<String> r2 = myTree.RangeSearchOnLowerBoundKey(79,99);
        for (String s:r2){
            System.out.print(s+" ");
        }
        System.out.println();
