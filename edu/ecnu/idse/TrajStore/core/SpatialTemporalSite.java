...
public class SpatialTemporalSite {
 ...
 public static final String spatialIndexPath = "hdfs://localhost:9000/user/zzg/spatialIndexCells";

    //    public static final String spatialIndexPath = "/home/zzg/IdeaProjects/TrajectoryStore/spatialIndexCells";
    /**
    ...
    public static void writeSpatialIndex(Configuration conf, CellInfo[] cellsInfo) throws IOException {
        FileSystem fs = FileSystem.get(URI.create(spatialIndexPath),conf);
      /*  do {
            tempFile = new Path("spatialIndexCells");
        } while (!fs.exists(tempFile));*/
        FSDataOutputStream out = fs.create(new Path(spatialIndexPath));
        /// total spaces number
    ...
    public static CellInfo[] ReadSpatialIndex(Configuration conf) throws IOException {
        ...
        Path indexPath = new Path(spatialIndexPath);
        FileSystem fs = indexPath.getFileSystem(conf);
        if (fs.exists(indexPath)){
            FSDataInputStream in = FileSystem.get(conf).open(indexPath);
            ...
            System.out.println("spatial index file"+ indexPath.getName()+" does not exists!");
        }
        return cells;
    }
